...
class CameraOdometerError {
 public:
  CameraOdometerError(Eigen::Vector3d r1, Eigen::Vector3d t1, Eigen::Vector3d r2, Eigen::Vector3d t2)
  ...
  bool operator()(const T* const q4x1, const T* const t3x1, const T* const scale, T* residuals) const {
    ...
    Eigen::Matrix<T, 3, 1> t_err = (R1 - Eigen::Matrix<T, 3, 3>::Identity()) * t - (scale[0] * R * t2) + t1;
...
CamOdoCalibration::CamOdoCalibration() : mMinMotions(200), mVerbose(false) {}

bool CamOdoCalibration::addMotionSegment(
    const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& Hs_odo,
    const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& Hs_cam, bool is_relative_position) {
  if (Hs_odo.size() != Hs_cam.size()) {
    return false;
  }

  MotionSegment segment;
  size_t begin_index = is_relative_position ? 0 : 1;
  for (size_t i = begin_index; i < Hs_odo.size(); ++i) {
    Eigen::Matrix4d H_odo, H_cam;
    // 1. transfrom to relative motion
    if (is_relative_position == false) {
      H_odo = Hs_odo.at(i - 1).inverse() * Hs_odo.at(i);  // o[i] T o[i+1]
      H_cam = Hs_cam.at(i - 1).inverse() * Hs_cam.at(i);  // c[i] T c[i+1]
    } else {
      H_odo = Hs_odo.at(i);
      H_cam = Hs_cam.at(i);
    }
...
size_t CamOdoCalibration::getMotionCount(void) const { return mMinMotions; }

void CamOdoCalibration::setMotionCount(size_t count) { mMinMotions = count; }

bool CamOdoCalibration::motionsEnough(void) const { return getCurrentMotionCount() >= getMotionCount(); }

bool CamOdoCalibration::calibrate(Eigen::Matrix4d& H_cam_odo) {
  std::vector<double> scales;

  std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>> rvecsOdo, tvecsOdo, rvecsCam,
      tvecsCam;
...
bool CamOdoCalibration::readMotionSegmentsFromFile(const std::string& filename) {
...
bool CamOdoCalibration::writeMotionSegmentsToFile(const std::string& filename) const {
...
bool CamOdoCalibration::getVerbose(void) { return mVerbose; }

void CamOdoCalibration::setVerbose(bool on) { mVerbose = on; }

bool CamOdoCalibration::estimate(
  ...
  // 1. solves for the pitch and roll components, R_yx
  Eigen::Matrix3d R_yx;
...
bool CamOdoCalibration::estimateRyx(
    const std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>>& rvecs1,
    const std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>>& tvecs1,
    const std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>>& rvecs2,
    const std::vector<std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>>& tvecs2,
...
void CamOdoCalibration::refineEstimate(

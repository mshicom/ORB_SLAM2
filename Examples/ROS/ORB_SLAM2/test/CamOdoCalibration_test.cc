...
TEST(CamOdoCalibration, MultiScale1) {
  ...
  for (int i = 0; i < segmentCount; ++i) {
    ...
    for (int j = 0; j < motionCount; ++j) {
      ...
      Eigen::Matrix3d R;
      R = Eigen::AngleAxisd(dyaw, Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(dpitch, Eigen::Vector3d::UnitY()) *
...
TEST(CamOdoCalibration, MultiScaleWithNoise) {
  ...
  std::cout << "Rotation error = "
            << r2d(acos(0.5 * ((H_12_expected.block<3, 3>(0, 0) * H_12.block<3, 3>(0, 0).transpose()).trace() - 1.0)))
            << " deg" << std::endl;

  Eigen::Vector2d t_expected = H_12_expected.block<2, 1>(0, 3);
  Eigen::Vector2d t = H_12.block<2, 1>(0, 3);

  std::cout << "Translation direction error = "
            << r2d(acos((t_expected.transpose() * t)(0, 0) / (t_expected.norm() * t.norm()))) << " deg" << std::endl;
  std::cout << "Translation error = " << (fabs(t_expected.norm() - t.norm()) / t_expected.norm()) * 100.0 << " %"
            << std::endl;
